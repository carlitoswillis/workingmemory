...
// file. Body: {text, list?} — list falls back to braindump, then the board's
// first live column. Auth: `Authorization: Bearer <BRAIN_TOKEN>`.
...
export async function POST(req: NextRequest) {
  ...
  let body: { text?: unknown; list?: unknown };
  try {
    body = await req.json();
  } catch {
  ...
  ).run(id, text, list, Date.now(), scope.ownerId, scope.boardId, scope.ownerId);
